...
local MENU_COLOR = Color3.fromRGB(255, 255, 255)
local BORDER = Color3.fromRGB(170, 220, 255)
...
local function drag(obj)
    ...
    UIS.InputChanged:Connect(function(i)
        if holding and (i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement) then
...
local ESP = false
local esp = make("ESP : OFF", 50)

local function updateESP()
    for _, p in ipairs(Players:GetPlayers()) do
        if p \~= Player and p.Character then
            local old = p.Character:FindFirstChild("DevDebug")
...
local inf = false
local jump = make("Infinite Jump : OFF", 95)

jump.MouseButton1Click:Connect(function()
    inf = not inf
    jump.Text = inf and "Infinite Jump : ON" or "Infinite Jump : OFF"
end)
...
local speedBtn = make("Speed : OFF", 140)
...
speedBtn.MouseButton1Click:Connect(function()
    enabled = not enabled
    speedBtn.Text = enabled and "Speed : ON" or "Speed : OFF"
    updateSpeed()
end)
...
value.Position = UDim2.new(0.1, 0, 0, 190)
value.Text = "16"
value.BackgroundColor3 = MENU_COLOR
...
local plus = make("+", 240)
plus.Size = UDim2.new(0.35, 0, 0, 35)
plus.Position = UDim2.new(0.1, 0, 0, 240)
...
local minus = make("-", 240)
minus.Size = UDim2.new(0.35, 0, 0, 35)
minus.Position = UDim2.new(0.55, 0, 0, 240)
